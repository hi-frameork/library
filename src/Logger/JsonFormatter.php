...
class JsonFormatter extends NormalizerFormatter
{
    ...
    public function format(LogRecord $record): string
    {
        ...
        // 将 trace 信息层级增加
        // 然后从 context 移除 _trace_ 减少数据干扰
        if ($trace = $normalized['context']['_trace_'] ?? null) {
            if (isset($trace['user_id'])) {
                $normalized['user_id'] = $trace['user_id'];
            }
            if (isset($trace['trace_id'])) {
                $normalized['trace_id'] = $trace['trace_id'];
            }
            if (isset($trace['point'])) {
                $normalized['point'] = $trace['point'];
            }

            unset($normalized['context']['_trace_']);
        }

        if (!$normalized['context']) {
            unset($normalized['context']);
        } else {
            $normalized['context'] = $this->toJson($normalized['context']);
        }
